...
### Fixed
- **`scripts/engine-gates.js` no longer returns `allowed:true` on any non-evaluable condition except a genuinely absent engine file**: previously, a corrupt/unparseable engine state file, an uncaught exception in `main()`, and a `--task` id matching zero enabled gates (e.g. a typo) all silently returned `allowed:true`, letting `/build` and `/ship` proceed past a gate that was never actually evaluated. Only "no engine state file on disk" is a legitimate non-engine workflow now — it returns `allowed:true, degraded:true` with a `notice`. Every other condition (corrupt/unloadable engine file, unrecognized `--task`, uncaught exception) now returns `allowed:false` with a `reason`, and both are written to stdout in the JSON payload (not stderr) since the consumer is the model reading stdout. Distinguishing "unrecognized task" from "task exists but has no gates targeting it" (a legitimate case, e.g. `task-clarify` in the build pipeline) required checking the target id against the engine's task list, not just counting matched gates. Existing fields (`allowed`, `blockers`, `warnings`) are unchanged; `degraded`, `notice`, and `reason` are additive. `warnings` is now always present in the payload, including on every pass-through/failure path. `commands/ship.md` doctrine updated: a gate's `allowed:false` verdict must be recorded in the run output rather than silently proceeded past, and degrading a block to a warning must likewise be recorded explicitly, never applied silently. `README.md`'s enforcement-engine claim narrowed from "Claude literally cannot skip Phase 4" to "...cannot skip Phase 4 silently" — the mechanism is a JSON verdict on stdout that the model reads and acts on; there is no hook-side enforcement. Covered by new `tests/engine-gates.test.js`.
...
### Fixed
- **Stop hook now honors `stop_hook_active`, gated on phase/milestone progress with a stall budget** (`hooks/handlers/stop-handler.js`): the hook previously never checked `stop_hook_active` on the parsed hook stdin. Per the vendor docs (https://code.claude.com/docs/en/hooks), `stop_hook_active` is `true` on every Stop event after the first block within a turn, and exists so a hook can "avoid blocking on a condition that will never resolve"; the runtime "overrides the hook and ends the turn after 8 consecutive blocks" unconditionally — no hook can extend or bypass that cap. The hook now persists `last_blocked_phase`, `last_blocked_milestone`, `last_blocked_fix_attempts`, and `stall_count` in the ship state frontmatter, and yields (stops blocking) once the pipeline looks stalled, so a stuck run gives back the rest of the 8-block budget instead of burning it on retries that make no progress. `phase` alone is not a sufficient progress signal — Phase 4 (Implement) loops over every milestone, with the Phase 5 verify loop nested inside, before `phase` is ever updated — so progress is "`phase` advanced OR `milestone` advanced OR `fix_attempts` advanced", and the guard only yields once `stall_count` exceeds a small heuristic budget (`STALL_LIMIT`, 3) of consecutive no-progress Stops, rather than on the very first one. A healthy run — including a multi-milestone stretch of a single phase, or a one-turn error-recovery pause — keeps blocking; only a genuine, sustained stall yields early. This does not change or extend the runtime's 8-block ceiling; it only avoids wasting that fixed budget on a stall.
